...
samples <- list.files("./obrien/gencode.v33lift37", full.names = TRUE)
...
write.table(txi$counts, file = "./obrien/gencode.v33lift37/tx.counts.tsv", 
            quote = FALSE, sep = "\t")

# Construct transcript to gene correspondence
annot <- read_delim("GENCODE/hg37/gencode.v33lift37.annotation.gtf.gz", 
                    delim = "\t", skip = 5, col_names = FALSE)
colnames(annot) <- c("chromosome", "source", "feature", "start", "end", 
                     "score", "strand", "phase", "info")
annot <- annot %>% 
...
write.table(txi.sum$counts, file = "./obrien/gencode.v33lift37/gene.counts.tsv", 
            quote = FALSE, sep = "\t")
```
